...
void loop() {
  ...
  if (START == 0) {
    ...
    // arm the UAV
    if ((input_THROTTLE < 1000) && (input_YAW < 1100) && (input_PITCH < 1000) && (input_ROLL > 1400) && (imu.available())) {
      START++;
    }
  }
  ...
  if (START == 1) {
    ...
    // dis-arm the UAV
    if ((input_THROTTLE < 1000) && (input_YAW < 1100) && (input_PITCH < 1000) && (input_ROLL > 1400)) {
      START--;
    }
    ...
    if ((input_THROTTLE < 1000) && (input_RESET > 1500)) { // hold current height
      //height control 
    }
  }
  maintain_loop_time();
